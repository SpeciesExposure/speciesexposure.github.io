---
title: "Species Exposure Dashboard"
format:
  dashboard:
    nav-buttons:
      - icon: github
        href: https://speciesexposure.github.io/
        aria-label: GitHub
---

```{=html}
<style>
#atlas-loading-overlay {
  position: fixed;
  inset: 0;
  z-index: 20000;
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 1.5rem;
  background: #fff;
  color: #2c3e50;
  transition: opacity 220ms ease, visibility 220ms ease;
}
#atlas-loading-overlay.is-hidden {
  opacity: 0;
  visibility: hidden;
  pointer-events: none;
}
#atlas-loading-overlay p {
  margin: 0;
  font-size: 1.1rem;
  font-weight: 600;
  text-align: center;
}
/* Prevent card focus state from resizing the map card */
.card:focus-within,
.bslib-card:focus-within {
  outline: none !important;
  box-shadow: none !important;
}
/* Prevent any flex child from growing beyond its allocated height.
   This stops the species table card from pushing the map row up.
   Every element in the flex chain must have min-height: 0. */
.bslib-gap-spacing,
.bslib-gap-spacing > *,
.html-fill-container,
.html-fill-item,
.bslib-card,
.bslib-card .card-body {
  min-height: 0 !important;
  min-width: 0 !important;
}
/* Cards in the bottom row scroll internally rather than expand */
.bslib-card .card-body {
  overflow: auto !important;
  overflow-x: hidden !important;
}
/* Force the leaflet widget and its wrappers to fill the card completely */
.bslib-card .card-body .html-widget,
.bslib-card .card-body .html-widget-output,
.bslib-card .card-body .leaflet,
.bslib-card .card-body .leaflet-container:not(.leaflet-control-minimap),
.leaflet:not(.leaflet-control-minimap),
.html-widget.leaflet-container:not(.leaflet-control-minimap) {
  width: 100% !important;
  height: 100% !important;
  grid-column: 1 / -1 !important;
}
/* Prevent bslib fill chain from collapsing the minimap */
.leaflet-control-minimap {
  flex: 0 0 auto !important;
}

/* ---- Home page: keep summary statistics content-sized in the scrolling layout */
#home-stats,
#home-stats > div,
#home-stats .html-fill-item {
  flex: 0 0 auto !important;
  height: auto !important;
}

/* ---- Species Explorer: cap polar+map row height ----
   The dashboard runs in scrolling layout so fr row-height units have no effect;
   each row sizes to its content.  Cap the polar+map row cards directly instead. */
#species-explorer .sidebar-content > .bslib-grid > .bslib-grid > .bslib-grid:nth-child(2) .bslib-card {
  max-height: 420px !important;
}

/* Tighten spacing between rows in Species Explorer */
#species-explorer .sidebar-content > .bslib-grid > .bslib-grid {
  row-gap: 4px !important;
}

/* ---- Hotspot Explorer: break bslib fill chain so explicit row heights work */
#hotspot-explorer .bslib-sidebar-layout,
#hotspot-explorer .main,
#hotspot-explorer .main > .bslib-grid {
  flex: 0 0 auto !important;
  height: auto !important;
}
.bslib-grid[data-height="525px"] {
  grid-template-rows: 525px 240px auto !important;
  height: auto !important;
}

/* On narrow viewports the bslib-grid collapses to a single column and the
   leaflet map card loses its declared row height (collapses to ~20px).
   Force a usable min-height for any card that contains a Leaflet widget. */
@media (max-width: 768px) {
  #hotspot-explorer .bslib-card:has(.leaflet),
  #hotspot-explorer .bslib-card:has(.html-widget) {
    min-height: 400px;
  }

  /* Stack sidebar above main content on phones (instead of the default
     left-edge collapsible drawer, which wastes screen real-estate and
     hides controls behind a toggle button). */
  .bslib-sidebar-layout {
    display: grid !important;
    grid-template-columns: 1fr !important;
    grid-template-rows: auto auto !important;
  }
  .bslib-sidebar-layout > .sidebar {
    grid-row: 1 !important;
    grid-column: 1 !important;
    width: 100% !important;
    max-width: none !important;
    transform: none !important;
    position: static !important;
    border-right: none !important;
    border-bottom: 1px solid #dee2e6 !important;
  }
  .bslib-sidebar-layout > .main {
    grid-row: 2 !important;
    grid-column: 1 !important;
  }
  /* Hide the collapse toggle since the sidebar is no longer a drawer */
  .bslib-sidebar-layout > .collapse-toggle,
  .bslib-sidebar-layout .collapse-toggle {
    display: none !important;
  }
}

/* ---- Species-Explorer sidebar: figure-selection checkbox grid (vars x aggs) */
.panel-checkbox-grid {
  border-collapse: collapse;
  font-size: 0.78rem;
  width: 100%;
  margin: 0 0 8px;
  background-color: transparent !important;
  --bs-table-bg: transparent;
  --bs-table-accent-bg: transparent;
  --bs-table-striped-bg: transparent;
  box-shadow: none !important;
}
.panel-checkbox-grid th,
.panel-checkbox-grid td,
.panel-checkbox-grid thead,
.panel-checkbox-grid tbody,
.panel-checkbox-grid tr {
  background-color: transparent !important;
  box-shadow: none !important;
}
.panel-checkbox-grid th,
.panel-checkbox-grid td {
  padding: 3px 6px;
  text-align: center;
  border-bottom: 1px solid #eee;
}
.panel-checkbox-grid thead th {
  font-weight: 600;
  color: #495057;
  border-bottom: 1px solid #ccc;
  white-space: nowrap;
  /* Rotate column headers so the grid fits a narrow sidebar */
  writing-mode: vertical-rl;
  transform: rotate(180deg);
  vertical-align: bottom;
  height: 5.5em;
  padding: 4px 2px;
  line-height: 1;
  background: transparent;
}
.panel-checkbox-grid tbody th {
  text-align: left;
  font-weight: 600;
  color: #495057;
  white-space: nowrap;
}
.panel-checkbox-grid tbody tr:hover {
  background: #f3f6fa;
}
.panel-checkbox-grid input[type="checkbox"] {
  cursor: pointer;
  margin: 0;
}

/* ---- Species-Explorer figure stack (one img per selected panel) */
.panel-figure-stack {
  display: flex;
  flex-direction: column;
  align-items: center;
  gap: 10px;
  width: 100%;
}
.panel-figure-stack figure {
  width: min(100%, 800px) !important;
}
.panel-figure-stack img {
  max-width: 100%;
  height: auto;
}
/* Pin the figure stack to the top of its card body (override any flex
   vertical centering inherited from bslib). */
#species-explorer .bslib-card .card-body:has(.panel-figure-stack) {
  justify-content: flex-start !important;
  align-items: stretch !important;
}

/* ---- Narrow-viewport fallback: collapse the checkbox grid into a list */
@media (max-width: 480px) {
  .panel-checkbox-grid thead { display: none; }
  .panel-checkbox-grid,
  .panel-checkbox-grid tbody,
  .panel-checkbox-grid tr,
  .panel-checkbox-grid td,
  .panel-checkbox-grid th { display: block; width: 100%; }
  .panel-checkbox-grid tbody th { padding-top: 6px; }
  .panel-checkbox-grid td {
    text-align: left;
    padding-left: 18px;
  }
  .panel-checkbox-grid td::before {
    content: attr(data-agg);
    margin-right: 8px;
    color: #777;
  }
}
</style>

<div id="atlas-loading-overlay" role="status" aria-live="polite">
  <p>Species exposure atlas loading, please be patient.</p>
</div>

<script>
window.addEventListener("load", () => {
  document.getElementById("atlas-loading-overlay")?.classList.add("is-hidden");
}, { once: true });
</script>
```

```{r setup, include=FALSE}
# =============================================================================
# Setup chunk – loaded once at render time.
#
# Reads the pre-computed cache files produced by _R/01_prepare-data.R.
# No heavy computation happens here; all summarisation was done at build time.
# =============================================================================

library(dplyr)
library(ggplot2)
library(plotly)
library(leaflet)
library(jsonlite)
library(leaflet.extras)
library(DT)
library(terra)

# Source plotting helpers
source("_R/timeseries-plotting.R")

# ---------------------------------------------------------------------------
# Load pre-computed cache files
# These are written by _R/01_prepare-data.R during the build step.
# ---------------------------------------------------------------------------

# Main exposure data: full table + year index + trend tables
dashboard_data <- readRDS("data/exposure-dashboard.rds")

exposure_df   <- dashboard_data$exposure_df
year_index    <- dashboard_data$year_index
cell_trend    <- dashboard_data$cell_trend
sp_trend      <- dashboard_data$sp_trend
years_avail   <- dashboard_data$years_avail
vars_avail    <- dashboard_data$vars_avail
species_avail <- dashboard_data$species_avail

# Species metadata (name, order, family, IUCN status)
species_metadata <- readRDS("data/species-metadata.rds")

# Per-species timeseries cache (keyed by species name)
ts_cache <- readRDS("data/species-timeseries-cache.rds")

# Land template raster (defines the spatial grid; cells match exposure_df$cell)
# Loaded from data/ where the build step should have copied it from the release
tpl <- terra::rast("data/landTemplate.tif")

# ---------------------------------------------------------------------------
# Shared constants
# ---------------------------------------------------------------------------

# Human-readable labels for climate variable codes
VAR_LABELS <- c(
  temp__12_up       = "Temperature: annual upper extreme",
  temp__12_lo       = "Temperature: annual lower extreme",
  temp__3__max_up   = "Temperature: seasonal max upper extreme",
  temp__3__min_lo   = "Temperature: seasonal min lower extreme",
  precip__12_up     = "Precipitation: annual upper extreme",
  precip__12_lo     = "Precipitation: annual lower extreme",
  precip__3__max_up = "Precipitation: seasonal max upper extreme",
  precip__3__min_lo = "Precipitation: seasonal min lower extreme"
)

# Colours for each climate variable (for map overlays and legends)
VAR_COLORS <- c(
  temp__12_up       = "#d73027",
  temp__12_lo       = "#4575b4",
  temp__3__max_up   = "#f46d43",
  temp__3__min_lo   = "#313695",
  precip__12_up     = "#1a9850",
  precip__12_lo     = "#9400D3",
  precip__3__max_up = "#33a02c",
  precip__3__min_lo = "#6a0dad"
)

# Helper: translate a variable code to its display label
var_label <- function(v) {
  lab <- VAR_LABELS[as.character(v)]
  dplyr::if_else(is.na(lab), as.character(v), lab)
}

# Helper: translate a variable code to its map colour
var_colour <- function(v) {
  col <- VAR_COLORS[as.character(v)]
  dplyr::if_else(is.na(col), "#e41a1c", col)
}

# Most recent year in the data (data always lags by ~1 year)
RECENT_YEAR   <- max(years_avail)           # e.g. 2025
RECENT_CUTOFF <- 2023L                      # first year of "Recent observed" period
hotspot_years <- RECENT_CUTOFF:RECENT_YEAR  # years shown in Hotspot Explorer

# Default selections shown on first load
default_year <- RECENT_YEAR
default_species <- species_avail[1]

# Named vector for the variable checkbox: label → code
var_choices <- setNames(vars_avail, vapply(vars_avail, var_label, character(1)))

# Species metadata passed to OJS for the dynamic species header
species_display <- species_metadata |>
  dplyr::rename(
    Species       = spName,
    Order         = dplyr::any_of("orderName"),
    Family        = dplyr::any_of("familyName"),
    `IUCN status` = dplyr::any_of("redlistCategory")
  )
ojs_define(species_meta_data = species_display)

# About the number of species and years for the home page
n_species_total <- nrow(species_metadata)
n_years_total   <- length(years_avail)
year_min        <- min(years_avail)
year_max        <- max(years_avail)
```

# Home {scrolling="true"}

## Row

### Col {width="65%"}

```{=html}
<div style="max-width:680px; padding: 1.5rem 0.5rem;">

<h2 style="margin-top:0.2rem;">Species Exposure Dashboard</h2>

<p style="font-size:1.05rem; line-height:1.6;">
This dashboard tracks how much of each species' geographic range has been
exposed to <strong>extreme climate events</strong> — temperatures and
precipitation outside the historical baseline — using ERA5 reanalysis data
from 1940 to the present.
</p>

<hr style="margin: 1.2rem 0;">

<h4>What you can explore</h4>
<ul style="line-height:1.8;">
  <li>
    <strong>Species Explorer</strong> — select any species to view its
    historical exposure time series, a polar summary of which climate
    variables have affected it most, and a map of its geographic range.
  </li>
  <li>
    <strong>Hotspot Explorer</strong> — click any grid cell on the global
    map to see all species present there and how many are currently
    exposed to extreme conditions.
  </li>
  <li>
    <strong>Shiny app</strong> — for more flexible filtering and custom data exploration, download and run the full interactive Shiny app from
    <a href="https://github.com/SpeciesExposure/exposureApp" target="_blank">github.com/SpeciesExposure/exposureApp</a>. <img src="img/shinyApp.png" alt="Screenshot of the Shiny app showing a species timeseries and map" style="max-width:50%; border:1px solid #ddd; border-radius:4px; margin:1rem 0;">
  </li>
</ul>

<hr style="margin: 1.2rem 0;">

<h4>Climate variables tracked</h4>
<p>Eight ERA5-derived extremes, covering both tails of temperature and
precipitation at annual and seasonal timescales:</p>
<ul style="line-height:1.8; columns:2; column-gap:2rem;">
  <li>High temperature (annual)</li>
  <li>Low temperature (annual)</li>
  <li>High temperature (warmest 3 months)</li>
  <li>Low temperature (coldest 3 months)</li>
  <li>High precipitation (annual)</li>
  <li>Low precipitation (annual)</li>
  <li>High precipitation (wettest 3 months)</li>
  <li>Low precipitation (driest 3 months)</li>
</ul>

<hr style="margin: 1.2rem 0;">

<h4>Data sources</h4>
<ul style="line-height:1.8;">
  <li><a href="https://www.ecmwf.int/en/forecasts/dataset/ecmwf-reanalysis-v5" target="_blank">ERA5 reanalysis</a> (ECMWF) — climate variables on a 0.25° global grid</li>
  <li><a href="https://www.iucnredlist.org/" target="_blank">IUCN Red List</a> — species range polygons and conservation status</li>
</ul>

<hr style="margin: 1.2rem 0;">



<p style="margin-top:1.5rem; font-size:0.85rem; color:#777;">
  Source code:
  <a href="https://github.com/SpeciesExposure" target="_blank">https://github.com/SpeciesExposure</a>
</p>

</div>
```

### Col {width="35%"}

```{r home-stats, echo=FALSE}
#| padding: 0
htmltools::tagList(
  htmltools::tags$div(
    id = "home-stats",
    style = paste0(
      "display:flex; flex-direction:column; gap:1rem; ",
      "padding:1rem 0.5rem; max-width:280px;"
    ),
    htmltools::tags$div(
      style = paste0(
        "background:#f8f9fa; border-left:4px solid #2c7bb6; ",
        "border-radius:4px; padding:1rem 1.2rem;"
      ),
      htmltools::tags$div(
        style = "font-size:2rem; font-weight:700; color:#2c7bb6; line-height:1.1;",
        format(n_species_total, big.mark = ",")
      ),
      htmltools::tags$div(
        style = "font-size:0.85rem; color:#555; margin-top:0.2rem;",
        "species with range maps"
      )
    ),
    htmltools::tags$div(
      style = paste0(
        "background:#f8f9fa; border-left:4px solid #1a9850; ",
        "border-radius:4px; padding:1rem 1.2rem;"
      ),
      htmltools::tags$div(
        style = "font-size:2rem; font-weight:700; color:#1a9850; line-height:1.1;",
        n_years_total
      ),
      htmltools::tags$div(
        style = "font-size:0.85rem; color:#555; margin-top:0.2rem;",
        paste0("years of ERA5 data (", year_min, "\u2013", year_max, ")")
      )
    ),
    htmltools::tags$div(
      style = paste0(
        "background:#f8f9fa; border-left:4px solid #d73027; ",
        "border-radius:4px; padding:1rem 1.2rem;"
      ),
      htmltools::tags$div(
        style = "font-size:2rem; font-weight:700; color:#d73027; line-height:1.1;",
        "8"
      ),
      htmltools::tags$div(
        style = "font-size:0.85rem; color:#555; margin-top:0.2rem;",
        "climate extreme variables"
      )
    ),
    htmltools::tags$div(
      style = paste0(
        "background:#f8f9fa; border-left:4px solid #756bb1; ",
        "border-radius:4px; padding:1rem 1.2rem;"
      ),
      htmltools::tags$div(
        style = "font-size:2rem; font-weight:700; color:#756bb1; line-height:1.1;",
        "0.25°"
      ),
      htmltools::tags$div(
        style = "font-size:0.85rem; color:#555; margin-top:0.2rem;",
        "spatial resolution (~28 km)"
      )
    )
  )
)
```

# Species Explorer {.tabset}

## {.sidebar}

**Species Explorer**

Select a species to see how much of its
range has been exposed to extreme climate events through time.

```{r species-sidebar, echo=FALSE}
# Sidebar controls are standard Quarto/HTML inputs connected to
# ojs (Observable JS) reactive blocks below via `ojs_define()`.

# Colour map: display label → hex, matching VAR_COLORS
var_label_colors_r <- setNames(
  vapply(unname(var_choices), function(code) {
    col <- VAR_COLORS[[code]]; if (is.na(col)) "#cccccc" else col
  }, character(1)),
  names(var_choices)
)

# Panel matrix for the Species-Explorer figure selector (variable x aggregation)
# Must stay in sync with PANEL_SLUG_MAP / RASTER_CONFIG in _R/panel-figures.R.
# Pass as a data.frame so ojs_define() ships it in column-major form and
# OJS `transpose()` can rehydrate the row records.
panel_matrix_r <- data.frame(
  family      = c("Temperature",   "Temperature",   "Temperature",
                  "Precipitation", "Precipitation", "Precipitation"),
  family_slug = c("temperature",   "temperature",   "temperature",
                  "precipitation", "precipitation", "precipitation"),
  agg         = c("Seasonal max", "Annual", "Seasonal min",
                  "Seasonal max", "Annual", "Seasonal min"),
  agg_slug    = c("seasonal_max", "annual", "seasonal_min",
                  "seasonal_max", "annual", "seasonal_min"),
  slug        = c("temperature__seasonal_max",   "temperature__annual",   "temperature__seasonal_min",
                  "precipitation__seasonal_max", "precipitation__annual", "precipitation__seasonal_min"),
  stringsAsFactors = FALSE
)

# Mapping from panel slug -> array of underlying variable *labels* (e.g.
# "Temperature: annual upper extreme"). Used by the Hotspot Explorer to
# translate the shared 2x3 panel-selection grid into the set of variable
# labels its cell_trend / cell_species filters expect. Mirrors RASTER_CONFIG
# (up_var + lo_var) in _R/panel-figures.R.
# I() wraps each entry so jsonlite serializes length-1 vectors as JS arrays
# (not unboxed scalar strings, which would break OJS flatMap downstream).
panel_var_labels_r <- list(
  "temperature__seasonal_max"   = I(unname(vapply(c("temp__3__max_up"),                var_label, character(1)))),
  "temperature__annual"         = I(unname(vapply(c("temp__12_up", "temp__12_lo"),     var_label, character(1)))),
  "temperature__seasonal_min"   = I(unname(vapply(c("temp__3__min_lo"),                var_label, character(1)))),
  "precipitation__seasonal_max" = I(unname(vapply(c("precip__3__max_up"),              var_label, character(1)))),
  "precipitation__annual"       = I(unname(vapply(c("precip__12_up", "precip__12_lo"), var_label, character(1)))),
  "precipitation__seasonal_min" = I(unname(vapply(c("precip__3__min_lo"),              var_label, character(1))))
)

# Pass R vectors to the OJS layer so controls can be populated dynamically
ojs_define(
  species_list       = species_avail,
  vars_list          = unname(var_choices),
  var_names          = names(var_choices),
  years_list         = years_avail,
  hotspot_years_list = hotspot_years,
  default_year       = default_year,
  var_label_colors   = as.list(var_label_colors_r),
  panel_matrix       = panel_matrix_r,
  panel_var_labels   = panel_var_labels_r
)
```

```{ojs}
//| echo: false
viewof selected_species = {
  // Load Tom Select CSS once (can't use require() for CSS)
  if (!document.querySelector('link[href*="tom-select"]')) {
    const link = document.createElement('link');
    link.rel = 'stylesheet';
    link.href = 'https://cdn.jsdelivr.net/npm/tom-select@2.4.3/dist/css/tom-select.bootstrap5.min.css';
    document.head.appendChild(link);
  }
  // OJS require() wraps CommonJS exports as { default: ... }
  const TomSelectModule = await require("https://cdn.jsdelivr.net/npm/tom-select@2.4.3/dist/js/tom-select.complete.min.js");
  const TomSelect = TomSelectModule.default ?? TomSelectModule;

  const metaRows = transpose(species_meta_data);
  const metadataBySpecies = new Map(metaRows.map(row => [row.Species, row]));
  const normalizeTaxon = value => value == null || String(value).trim() === ""
    ? "Unknown"
    : String(value);
  const catalogue = species_list.map(species => {
    const metadata = metadataBySpecies.get(species) ?? {};
    return {
      value: species,
      text: species.replace(/_/g, " "),
      order: normalizeTaxon(metadata.Order),
      family: normalizeTaxon(metadata.Family)
    };
  });
  const orders = [...new Set(catalogue.map(row => row.order))]
    .sort((a, b) => a.localeCompare(b));

  const makeLabel = (text, marginTop = "0") => {
    const label = document.createElement("label");
    label.textContent = text;
    label.style.cssText = `display:block;font-size:0.75rem;font-weight:600;text-transform:uppercase;margin:${marginTop} 0 4px;color:#495057;`;
    return label;
  };
  const addChoice = (select, value, text) => {
    const option = document.createElement("option");
    option.value = value;
    option.textContent = text;
    select.appendChild(option);
  };

  // Wrapper that OJS viewof reads .value from and listens to for input events
  const wrapper = document.createElement("div");
  const orderSelect = document.createElement("select");
  const familySelect = document.createElement("select");
  const speciesSelect = document.createElement("select");

  addChoice(orderSelect, "", "All orders");
  orders.forEach(order => addChoice(orderSelect, order, order));
  addChoice(familySelect, "", "All families");

  wrapper.appendChild(makeLabel("Order"));
  wrapper.appendChild(orderSelect);
  wrapper.appendChild(makeLabel("Family", "8px"));
  wrapper.appendChild(familySelect);
  wrapper.appendChild(makeLabel("Species", "8px"));
  wrapper.appendChild(speciesSelect);
  Object.defineProperty(wrapper, "value", { get: () => speciesSelect.value });
  yield wrapper;

  const speciesControl = new TomSelect(speciesSelect, {
    valueField: "value",
    labelField: "text",
    searchField: ["text"],
    optgroupField: "order",
    optgroupValueField: "value",
    optgroupLabelField: "label",
    lockOptgroupOrder: true,
    maxOptions: 100,
    placeholder: "Type to search species...",
    dropdownParent: "body",
    render: {
      no_results: () => '<div class="no-results">No matching species</div>'
    },
    onChange: () => wrapper.dispatchEvent(new Event("input", { bubbles: true }))
  });

  const rebuildFamilies = () => {
    const selectedOrder = orderSelect.value;
    const previousFamily = familySelect.value;
    const families = new Map();
    for (const row of catalogue) {
      if (selectedOrder && row.order !== selectedOrder) continue;
      const key = `${row.order}\u001f${row.family}`;
      const label = selectedOrder ? row.family : `${row.family} — ${row.order}`;
      families.set(key, label);
    }

    familySelect.replaceChildren();
    addChoice(familySelect, "", "All families");
    [...families.entries()]
      .sort(([, a], [, b]) => a.localeCompare(b))
      .forEach(([value, label]) => addChoice(familySelect, value, label));
    familySelect.value = families.has(previousFamily) ? previousFamily : "";
  };

  const rebuildSpecies = () => {
    const selectedOrder = orderSelect.value;
    const selectedFamily = familySelect.value;
    const previousSpecies = speciesControl.getValue();
    const filtered = catalogue.filter(row => {
      const familyKey = `${row.order}\u001f${row.family}`;
      return (!selectedOrder || row.order === selectedOrder) &&
        (!selectedFamily || familyKey === selectedFamily);
    });

    speciesControl.clear(true);
    speciesControl.clearOptions();
    speciesControl.clearOptionGroups();
    [...new Set(filtered.map(row => row.order))]
      .sort((a, b) => a.localeCompare(b))
      .forEach(order => speciesControl.addOptionGroup(order, { value: order, label: order }));
    speciesControl.addOptions(filtered);

    const nextSpecies = filtered.some(row => row.value === previousSpecies)
      ? previousSpecies
      : filtered[0]?.value ?? "";
    speciesControl.setValue(nextSpecies, true);
    speciesControl.refreshOptions(false);
    if (nextSpecies !== previousSpecies) {
      wrapper.dispatchEvent(new Event("input", { bubbles: true }));
    }
  };

  orderSelect.addEventListener("change", () => {
    rebuildFamilies();
    rebuildSpecies();
  });
  familySelect.addEventListener("change", rebuildSpecies);

  rebuildFamilies();
  rebuildSpecies();

  invalidation.then(() => speciesControl.destroy());
}
```

```{ojs}
//| echo: false
//| output: false
// Reusable factory for the 2 (families) x 3 (aggregations) panel-selection
// checkbox grid. Returns a DOM element that satisfies the viewof contract
// (exposes `.value` as the array of selected slug strings and dispatches
// `input` events on change).
makePanelCheckboxGrid = function({ defaults = [], label = "Figures" } = {}) {
  const rows = transpose(panel_matrix);
  const families = ["Temperature", "Precipitation"];
  const aggs     = ["Seasonal max", "Annual", "Seasonal min"];
  const defaultSet = new Set(defaults);

  const wrapper = document.createElement("div");
  const lbl = document.createElement("label");
  lbl.textContent = label;
  lbl.style.cssText = "display:block;font-size:0.75rem;font-weight:600;letter-spacing:0.05em;text-transform:uppercase;margin:8px 0 4px;color:#495057;";
  wrapper.appendChild(lbl);

  const table = document.createElement("table");
  table.className = "panel-checkbox-grid";

  // Header row
  const thead = document.createElement("thead");
  const hr = document.createElement("tr");
  hr.appendChild(document.createElement("th"));
  for (const a of aggs) {
    const th = document.createElement("th");
    th.textContent = a;
    hr.appendChild(th);
  }
  thead.appendChild(hr);
  table.appendChild(thead);

  // Body rows
  const tbody = document.createElement("tbody");
  const checkboxes = [];
  for (const fam of families) {
    const tr = document.createElement("tr");
    const rh = document.createElement("th");
    rh.scope = "row";
    rh.textContent = fam;
    tr.appendChild(rh);
    for (const a of aggs) {
      const td = document.createElement("td");
      td.setAttribute("data-agg", a);
      const cell = rows.find(r => r.family === fam && r.agg === a);
      if (cell) {
        const cb = document.createElement("input");
        cb.type = "checkbox";
        cb.value = cell.slug;
        cb.checked = defaultSet.has(cell.slug);
        cb.setAttribute("aria-label", `${fam} ${a}`);
        td.appendChild(cb);
        checkboxes.push(cb);
      }
      tr.appendChild(td);
    }
    tbody.appendChild(tr);
  }
  table.appendChild(tbody);
  wrapper.appendChild(table);

  Object.defineProperty(wrapper, "value", {
    get: () => checkboxes.filter(c => c.checked).map(c => c.value)
  });
  for (const cb of checkboxes) {
    cb.addEventListener("change", () => {
      wrapper.dispatchEvent(new Event("input", { bubbles: true }));
    });
  }
  return wrapper;
}
```

```{ojs}
//| echo: false
viewof selected_panels = makePanelCheckboxGrid({
  defaults: ["temperature__annual"],
  label: "Figures"
})
```

---

*Data: historical observations up to 2025*

## Species

### Row {height="70px"}

```{ojs}
//| echo: false
{
  const meta = transpose(species_meta_data).filter(row => row.Species === selected_species)[0];
  if (!meta) return html`<div></div>`;
  const sp = (meta.Species || "").replace(/_/g, " ");
  return html`<div style="padding:6px 16px;display:flex;align-items:center;gap:24px;">
    <h4 style="margin:0;font-style:italic;">${sp}</h4>
    <span style="color:#555;">Order: <strong>${meta.Order ?? "\u2014"}</strong></span>
    <span style="color:#555;">Family: <strong>${meta.Family ?? "\u2014"}</strong></span>
    <span style="color:#555;">IUCN: <strong>${meta["IUCN status"] ?? "\u2014"}</strong></span>
  </div>`;
}
```

### Row {height="420px"}

#### Polar {width="40%"}

```{ojs}
//| echo: false
{
  const sp  = encodeURIComponent(selected_species);

  // Outer wrapper holds the image + expand button
  const outer = DOM.element("div");
  outer.style.cssText = "position:relative;width:100%;height:100%;display:flex;align-items:center;justify-content:center;";

  const img = DOM.element("img");
  img.src = `data/species_figs/${sp}_polar.png`;
  img.style.cssText = "max-width:100%;max-height:160px;object-fit:contain;display:block;";
  img.onerror = () => {
    const msg = DOM.element("p");
    msg.textContent = "Polar figure not yet built \u2014 run the build step.";
    msg.style.cssText = "color:#999;font-size:12px;text-align:center;margin-top:2em;";
    img.replaceWith(msg);
  };
  outer.appendChild(img);

  // Expand button — opens a modal lightbox with the full-size image
  const btn = DOM.element("button");
  btn.title = "Expand";
  btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>`;
  btn.style.cssText = "position:absolute;top:4px;right:4px;padding:3px 5px;background:rgba(255,255,255,0.88);border:1px solid #bbb;border-radius:4px;cursor:pointer;line-height:1;color:#555;z-index:10;";
  btn.addEventListener("click", () => {
    const overlay = DOM.element("div");
    overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.78);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;";
    const panel = DOM.element("div");
    panel.style.cssText = "background:#fff;border-radius:8px;padding:16px;box-shadow:0 6px 32px rgba(0,0,0,0.6);cursor:default;";
    panel.addEventListener("click", e => e.stopPropagation());
    const full = DOM.element("img");
    full.src = img.src;
    full.style.cssText = "max-width:calc(90vw - 32px);max-height:calc(90vh - 32px);object-fit:contain;display:block;";
    panel.appendChild(full);
    overlay.appendChild(panel);
    const close = () => { if (document.body.contains(overlay)) document.body.removeChild(overlay); };
    overlay.addEventListener("click", close);
    document.addEventListener("keydown", function esc(e) {
      if (e.key === "Escape") { close(); document.removeEventListener("keydown", esc); }
    });
    document.body.appendChild(overlay);
  });
  outer.appendChild(btn);

  yield outer;
  // After two rAF ticks, measure the card and update the image max-height.
  await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
  const _card = outer.closest('.bslib-card');
  const _hdr  = _card?.querySelector('.card-header');
  const _availH = (_card?.getBoundingClientRect().height ?? 0)
                - (_hdr?.getBoundingClientRect().height  ?? 0);
  if (_availH > 20) img.style.maxHeight = _availH + 'px';
}
```

#### Range map {width="60%"}

```{ojs}
//| echo: false
species_range_map = {
  // Outer wrapper: initial height is a fallback; a ResizeObserver resizes it
  // to fill the card body after yield (.observablehq parent has no height).
  const outer = DOM.element("div");
  outer.style.cssText = "width:100%;height:280px;";
  const container = DOM.element("div");
  container.style.cssText = "width:100%;height:100%;";
  outer.appendChild(container);
  yield outer;

  // Size outer to fill the card body (wait two rAF ticks for layout to settle)
  await new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
  const _card = outer.closest('.bslib-card');
  const _hdr  = _card?.querySelector('.card-header');
  const _availH = (_card?.getBoundingClientRect().height ?? 0)
                - (_hdr?.getBoundingClientRect().height  ?? 0);
  if (_availH > 50) outer.style.height = _availH + 'px';

  let map = null;

  // IUCN terms of use prohibit redistribution of reptile and amphibian range data.
  const RESTRICTED_ORDERS = new Set([
    "SQUAMATA","TESTUDINES","CROCODYLIA","RHYNCHOCEPHALIA", // reptiles
    "ANURA","CAUDATA","GYMNOPHIONA"                         // amphibians
  ]);
  const spMeta = transpose(species_meta_data).filter(row => row.Species === selected_species)[0];
  if (RESTRICTED_ORDERS.has(spMeta?.Order)) {
    const msg = DOM.element("p");
    msg.innerHTML = `Sorry, the IUCN data use agreement does not permit showing the range of this species &mdash; see <a href="https://www.iucnredlist.org/terms/terms-of-use" target="_blank" rel="noopener">IUCN terms of use (§ No Reposting or Redistribution)</a>.`;
    msg.style.cssText = "color:#666;font-size:13px;text-align:center;margin-top:3em;padding:0 1.5em;line-height:1.5;";
    container.appendChild(msg);
    return;
  }

  // Use the Leaflet already loaded by the R leaflet package (site_libs/leaflet-1.3.1).
  // Loading a separate CDN copy would overwrite window.L and break the Hotspot
  // Explorer's addProviderTiles call.
  const L = window.L;

  const sp = encodeURIComponent(selected_species);
  let mapData;
  try {
    const resp = await fetch(`data/species_maps/${sp}_map.json`);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    mapData = await resp.json();
  } catch(e) {
    const msg = DOM.element("p");
    msg.textContent = "Range map not yet built \u2014 run the build step.";
    msg.style.cssText = "color:#999;font-size:12px;text-align:center;margin-top:2em;";
    container.appendChild(msg);
    return;
  }

  map = L.map(container, { preferCanvas: true });
  L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}", {
    attribution: "Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ",
    maxZoom: 19
  }).addTo(map);

  const renderer = L.canvas({ padding: 0.5 });
  const bounds = [];

  // Colour: 1 variable = blue, 8 variables = red
  const varColor = (v) => {
    const t = Math.min((v - 1) / 7, 1);
    const r = Math.round(t * 215 + (1 - t) * 49);
    const g = Math.round((1 - t) * 115);
    const b = Math.round((1 - t) * 178 + t * 39);
    return `rgb(${r},${g},${b})`;
  };

  for (const [c, v] of mapData) {
    const col = (c - 1) % rast_ncols + 1;
    const row = Math.floor((c - 1) / rast_ncols) + 1;
    const lon = rast_xmin + (col - 0.5) * rast_xres;
    const lat = rast_ymax - (row - 0.5) * rast_yres;
    const half_x = rast_xres / 2;
    const half_y = rast_yres / 2;
    L.rectangle(
      [[lat - half_y, lon - half_x], [lat + half_y, lon + half_x]],
      { fillColor: varColor(v), color: "none", weight: 0, fillOpacity: 0.75, renderer }
    ).addTo(map);
    bounds.push([lat, lon]);
  }

  if (bounds.length)
    map.fitBounds(L.latLngBounds(bounds), { padding: [10, 10], maxZoom: 5 });

  // Sync Leaflet to the resized container (height was set after map init)
  map.invalidateSize();

  // Add minimap using the same plugin bundled by the R leaflet package.
  if (!window.L?.Control?.MiniMap) {
    if (!document.querySelector('link[href*="Control.MiniMap"]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = 'site_libs/leaflet-minimap-3.3.1/Control.MiniMap.min.css';
      document.head.appendChild(link);
    }
    await new Promise((resolve, reject) => {
      const s = document.createElement('script');
      s.src = 'site_libs/leaflet-minimap-3.3.1/Control.MiniMap.min.js';
      s.onload = resolve; s.onerror = reject;
      document.head.appendChild(s);
    });
  }
  new L.Control.MiniMap(
    L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}", { maxZoom: 19 }),
    { toggleDisplay: true, position: "topright", minimized: false }
  ).addTo(map);

  invalidation.then(() => map.remove());
}
```

### Row

```{r species-figures-data, echo=FALSE}
ojs_define(has_species_figs = file.exists("data/species_figs"))
```

```{ojs}
//| echo: false
{
  const sp     = encodeURIComponent(selected_species);
  const panels = transpose(panel_matrix);
  const slugToLabel = new Map(panels.map(p => [p.slug, `${p.family} \u2014 ${p.agg}`]));

  const wrapper = DOM.element("div");
  wrapper.style.cssText = "display:flex;flex-direction:column;width:100%;";

  const stack = DOM.element("div");
  stack.className = "panel-figure-stack";
  wrapper.appendChild(stack);

  if (!selected_panels || selected_panels.length === 0) {
    const msg = DOM.element("p");
    msg.textContent = "Select one or more figures from the sidebar.";
    msg.style.cssText = "color:#999;font-size:12px;text-align:center;margin:1.5em 0;";
    stack.appendChild(msg);
  }

  // Lightbox helper (modal full-size image on click)
  const openLightbox = (src, alt) => {
    const overlay = DOM.element("div");
    overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.78);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:zoom-out;padding:24px;";
    const big = DOM.element("img");
    big.src = src; big.alt = alt;
    big.style.cssText = "max-width:95vw;max-height:90vh;background:#fff;padding:12px;border-radius:8px;box-shadow:0 6px 32px rgba(0,0,0,0.6);";
    overlay.appendChild(big);
    overlay.addEventListener("click", () => overlay.remove());
    document.body.appendChild(overlay);
  };

  for (const slug of selected_panels) {
    const figure = DOM.element("figure");
    figure.style.cssText = "margin:0;width:100%;";

    const img = DOM.element("img");
    const src = `data/species_figs/${sp}__${slug}.png`;
    img.src = src;
    img.alt = `${selected_species.replace(/_/g, " ")} \u2014 ${slugToLabel.get(slug) ?? slug}`;
    img.title = "Click to enlarge";
    img.style.cssText = "max-width:100%;height:auto;display:block;margin:0 auto;cursor:zoom-in;";
    img.onerror = () => {
      const msg = DOM.element("p");
      msg.textContent = `No figure built for ${slugToLabel.get(slug) ?? slug}.`;
      msg.style.cssText = "color:#999;font-size:12px;text-align:center;margin:1em 0;";
      img.replaceWith(msg);
    };
    img.addEventListener("click", () => openLightbox(src, img.alt));

    figure.appendChild(img);
    stack.appendChild(figure);
  }

  const cap = DOM.element("div");
  cap.style.cssText = "flex:0 0 auto;font-size:0.78rem;line-height:1.55;color:#444;padding:0.6rem 1rem 0.4rem;border-top:1px solid #dee2e6;margin-top:4px;";
  cap.innerHTML = `<strong>Figure.</strong>
Per-cell climate exposure across the geographic range of the selected species, 1941\u20132025.
Each selected panel shows one ERA5-derived variable (temperature or precipitation) at one temporal aggregation (annual or warmest/coldest 3-month season).
<strong>Left sub-panel \u2014 Value \u00d7 time heatmap.</strong>
Year (x-axis) vs. the per-cell variable value (y-axis); tile fill encodes the percentile of that value under the historical empirical CDF (1941\u20132022), pooled across all range cells.
Translucent lines trace year-to-year trajectories for a random subsample of range cells; segments are coloured when the cell\u2019s value exceeds the species-level upper threshold (99th-of-99th-percentiles) or falls below the lower threshold (1st-of-1st-percentiles).
Dashed horizontal lines mark these thresholds.
<strong>Right sub-panel \u2014 Kernel density estimates.</strong>
Normalized KDEs of the variable, pooled across range cells, for the historical baseline period (grey) and the most recent year (purple); vertical dashed lines repeat the exposure thresholds.`;
  wrapper.appendChild(cap);

  return wrapper;
}
```

# Hotspot Explorer {scrolling="true"}

## {.sidebar}

**Hotspot Explorer**

Click any cell on the map to see which species are exposed there.

```{ojs}
//| echo: false
viewof hotspot_year = Inputs.select(
  hotspot_years_list.map(String),
  { value: String(default_year), label: "Year" }
)
```

```{=html}
<p style="font-size:0.82em;color:#666;margin-bottom:4px;">
  Select which variables appear in the pixel-level exposure time series below the map.
</p>
```

```{ojs}
//| echo: false
viewof selected_hotspot_panels = makePanelCheckboxGrid({
  defaults: ["temperature__seasonal_max", "temperature__annual", "temperature__seasonal_min",
             "precipitation__seasonal_max", "precipitation__annual", "precipitation__seasonal_min"],
  label: "Variables"
})
```

```{ojs}
//| echo: false
//| output: false
// Translate the selected panel slugs into the underlying variable *labels*
// that the hotspot cell-trend and cell-species filters expect.
// Keeps the existing `.includes(d.var)` filtering logic working unchanged.
hotspot_vars = (selected_hotspot_panels ?? [])
  .flatMap(slug => [panel_var_labels[slug]].flat().filter(Boolean))
```

```{ojs}
//| echo: false
viewof hotspot_threatened_only = Inputs.toggle({
  label: "Only threatened species (CR / EN / VU)",
  value: false
})
```

## Row {height="525px"}

### Map

```{r hotspot-map, echo=FALSE}
# Build an interactive Leaflet map showing the hotspot raster.
# All years are pre-rendered as stacked image layers; JS toggles visibility.

# Build rasters for every available year using a shared colour domain
# so the colour scale is consistent across years.
all_hotspot_counts <- exposure_df |>
  dplyr::filter(!is.na(cell), cell >= 1, cell <= terra::ncell(tpl)) |>
  dplyr::distinct(spName, cell, year) |>
  dplyr::count(cell, year, name = "n_species")

global_vals <- all_hotspot_counts$n_species
global_pal  <- leaflet::colorNumeric(
  palette   = colorRampPalette(c("steelblue4", "steelblue1", "gold", "red1", "red4"))(100),
  domain    = log10(c(1, max(global_vals))),
  na.color  = "transparent"
)
# Wrapper: maps raw counts → log10 → colour
pal_fn <- function(x) global_pal(log10(pmax(x, 1)))
# Legend palette and values on log10 scale for readability
legend_breaks <- c(1, 2, 5, 10, 20, 50, 100, max(global_vals))
legend_breaks <- legend_breaks[legend_breaks <= max(global_vals)]
raster_ext    <- as.vector(terra::ext(tpl))
raster_res    <- terra::res(tpl)

# Start map
lmap <- leaflet::leaflet(height = "525px", options = leaflet::leafletOptions(preferCanvas = TRUE)) |>
  leaflet::addTiles(
    urlTemplate = "https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}",
    attribution = "Tiles &copy; Esri &mdash; Esri, DeLorme, NAVTEQ"
  ) |>
  leaflet::setView(lng = 0, lat = 15, zoom = 2) |>
  leaflet::addMiniMap(
    tiles         = "https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}",
    toggleDisplay = TRUE,
    position      = "topright",
    minimized     = FALSE
  )

# Add one vector-rectangle overlay per hotspot year, each in its own Leaflet
# pane rendered on canvas. Vector cells use the exact source lon/lat grid (no
# reprojection), so the fill always matches the click-selection math below.
# A canvas renderer (vs. the SVG default) avoids two problems: sub-pixel
# rectangles disappearing at low zoom, and hairline seams between cells.
# Cells are also expanded by a small epsilon so adjacent fills overlap
# slightly, as extra insurance against seams.
seam_eps <- min(raster_res) * 0.001
for (yr in hotspot_years) {
  pane_name <- paste0("hotspot_", yr)
  lmap <- lmap |> leaflet::addMapPane(pane_name, zIndex = 410)
  yr_counts <- all_hotspot_counts |> dplyr::filter(year == yr)
  cell_xy <- terra::xyFromCell(tpl, yr_counts$cell)
  lmap <- lmap |> leaflet::addRectangles(
    lng1 = cell_xy[, 1] - raster_res[1] / 2 - seam_eps,
    lat1 = cell_xy[, 2] - raster_res[2] / 2 - seam_eps,
    lng2 = cell_xy[, 1] + raster_res[1] / 2 + seam_eps,
    lat2 = cell_xy[, 2] + raster_res[2] / 2 + seam_eps,
    stroke = FALSE,
    fillColor = pal_fn(yr_counts$n_species),
    fillOpacity = 1,
    options = leaflet::pathOptions(
      interactive = FALSE,
      smoothFactor = 0, # disable point simplification so tiny cells don't vanish at low zoom
      noClip = TRUE,
      renderer = leaflet::JS(sprintf("L.canvas({pane: '%s', padding: 0.5})", pane_name))
    )
  )
}

# Dedicated pane (above the hotspot data panes) for the click-selection outline.
lmap <- lmap |> leaflet::addMapPane("cellSelectPane", zIndex = 620)

# Add legend and click handler
default_year_df <- exposure_df[year_index[[as.character(default_year)]], ]
lmap <- lmap |>
  leaflet::addLegend(
    position = "bottomright",
    colors   = global_pal(log10(pmax(legend_breaks, 1))),
    labels   = as.character(legend_breaks),
    title    = "# Species exposed",
    opacity  = 1
  ) |>
  # Wire map clicks to a custom JS event so OJS can react.
  # Use paste0 so hotspot_years is embedded as a JS array literal at render time.
  htmlwidgets::onRender(paste0("
    function(el, x) {
      var map = this;
      map.invalidateSize(false);
      setTimeout(function() { map.invalidateSize(false); }, 300);
      // Remove bslib fill classes from minimap so its inline width/height styles
      // are not overridden and the toggle button works correctly.
      setTimeout(function() {
        var mm = el.querySelector('.leaflet-control-minimap');
        if (mm) { mm.classList.remove('html-fill-item', 'html-fill-container'); }
      }, 100);
      // Listen for year changes from OJS and toggle raster layer opacity.
      var hotspotYears = [", paste(hotspot_years, collapse = ","), "];
      var rasterXmin = ", raster_ext[1], ";
      var rasterYmax = ", raster_ext[4], ";
      var rasterXres = ", raster_res[1], ";
      var rasterYres = ", raster_res[2], ";
      var rasterNcols = ", terra::ncol(tpl), ";
      var rasterNrows = ", terra::nrow(tpl), ";
      var setHotspotYear = function(year) {
        hotspotYears.forEach(function(y) {
          var pane = map.getPane('hotspot_' + y);
          if (pane) pane.style.display = String(y) === String(year) ? 'block' : 'none';
        });
      };
      setHotspotYear(", default_year, ");
      window.addEventListener('hotspot_year_change', function(e) {
        setHotspotYear(e.detail.year);
      });
      map.on('click', function(e) {
        // Snap directly to the source longitude/latitude grid.
        var col = Math.floor((e.latlng.lng - rasterXmin) / rasterXres);
        var row = Math.floor((rasterYmax - e.latlng.lat) / rasterYres);
        if (col < 0 || col >= rasterNcols || row < 0 || row >= rasterNrows) return;
        var west = rasterXmin + col * rasterXres;
        var north = rasterYmax - row * rasterYres;
        if (window._clickCell) { window._clickCell.remove(); }
        if (window._clickCellHalo) { window._clickCellHalo.remove(); }
        var cellBounds = [[north - rasterYres, west], [north, west + rasterXres]];
        // White halo drawn first so the purple line stays visible over any fill colour.
        window._clickCellHalo = L.rectangle(cellBounds,
          { color: '#ffffff', weight: 4, fill: false, interactive: false, pane: 'cellSelectPane' }
        ).addTo(map);
        window._clickCell = L.rectangle(cellBounds,
          { color: '#a020f0', weight: 2, fill: false, interactive: false, pane: 'cellSelectPane' }
        ).addTo(map);

        window._mapClick = {
          lat: north - rasterYres / 2,
          lng: west + rasterXres / 2,
          cell: row * rasterNcols + col + 1,
          t: Date.now()
        };
        window.dispatchEvent(new CustomEvent('leaflet_mapclick', { detail: window._mapClick }));
      });
    }
  "))
# Hide non-default year panes via CSS (not just JS) so the very first paint,
# before onRender fires, never briefly shows every year at once.
hotspot_pane_css <- paste0(
  sprintf(".leaflet-hotspot_%s-pane { display: none; }", setdiff(hotspot_years, default_year)),
  collapse = "\n"
)
htmltools::tagList(htmltools::tags$style(htmltools::HTML(hotspot_pane_css)), lmap)
```

```{r raster-geometry, include=FALSE}
# Pass raster geometry to OJS so the click handler can convert lat/lng → cell index.
# Terra cell numbering: row-major, 1-indexed, row 1 = northernmost.
rast_ext  <- as.vector(terra::ext(tpl))   # xmin, xmax, ymin, ymax
rast_res  <- terra::res(tpl)              # xres, yres (both positive)
ojs_define(
  rast_xmin  = rast_ext[1],
  rast_ymax  = rast_ext[4],
  rast_xres  = rast_res[1],
  rast_yres  = rast_res[2],
  rast_ncols = terra::ncol(tpl),
  rast_nrows = terra::nrow(tpl)
)
```

```{r cell-lookup-data, include=FALSE}
# Write compact per-year JSON files + small lookup files.
# Format: [{c: cell, s: spName, v: varIdx, p: propExposed, r: redlistIdx}]
# Lookup files (written once): var_codes.json, redlist_codes.json, species_attrs.json
# This reduces file size ~75% vs the old verbose format.
cell_species_dir <- "data/cell_species"
dir.create(cell_species_dir, recursive = TRUE, showWarnings = FALSE)

# --- build lookup tables ---
all_var_labels <- sort(unique(vapply(as.character(unique(exposure_df$var)), var_label, character(1))))
var_idx        <- setNames(seq_along(all_var_labels) - 1L, all_var_labels)
jsonlite::write_json(as.list(all_var_labels),
                     file.path(cell_species_dir, "var_codes.json"), auto_unbox = TRUE)

all_redlist  <- sort(unique(as.character(exposure_df$redlistCategory)))
redlist_idx  <- setNames(seq_along(all_redlist) - 1L, all_redlist)
jsonlite::write_json(as.list(all_redlist),
                     file.path(cell_species_dir, "redlist_codes.json"), auto_unbox = TRUE)

sp_attrs <- dplyr::distinct(exposure_df, spName, orderName, familyName) |>
  dplyr::filter(!is.na(spName))
sp_attrs_list <- setNames(
  lapply(seq_len(nrow(sp_attrs)), function(i)
    list(o = sp_attrs$orderName[i], f = sp_attrs$familyName[i])),
  sp_attrs$spName)
jsonlite::write_json(sp_attrs_list,
                     file.path(cell_species_dir, "species_attrs.json"), auto_unbox = TRUE)

# --- per-year compact files (Hotspot Explorer: recent years only) ---
for (yr in RECENT_CUTOFF:RECENT_YEAR) {
  yr_path <- file.path(cell_species_dir, paste0("species_", yr, ".json"))
  yr_data <- exposure_df[year_index[[as.character(yr)]], ] |>
    dplyr::filter(!is.na(cell), cell >= 1, cell <= terra::ncell(tpl)) |>
    dplyr::mutate(
      vl = vapply(as.character(var), var_label, character(1)),
      v  = var_idx[vl],
      p  = round(as.numeric(propExposed), 4),
      r  = redlist_idx[as.character(redlistCategory)]
    ) |>
    dplyr::arrange(cell, v) |>
    dplyr::select(c = cell, s = spName, v, p, r)
  jsonlite::write_json(yr_data, yr_path, auto_unbox = TRUE)
}

# Quarto copies static assets before executing this chunk. Sync the refreshed
# lookup files into the rendered site so OJS fetches the current data.
site_cell_species_dir <- file.path("_site", cell_species_dir)
dir.create(site_cell_species_dir, recursive = TRUE, showWarnings = FALSE)
file.copy(
  list.files(cell_species_dir, full.names = TRUE),
  site_cell_species_dir,
  overwrite = TRUE
)
```

```{ojs}
//| output: false
// Listen for leaflet_mapclick events dispatched by the onRender handler.
// Converts lat/lng to a terra cell index (row-major, 1-indexed).
clicked_cell = Generators.observe(notify => {
  function handler(e) {
    const { lat, lng } = e.detail;
    if (e.detail.cell) {
      notify(String(e.detail.cell));
      return;
    }
    if (lng < rast_xmin || lng > (rast_xmin + rast_ncols * rast_xres) ||
        lat > rast_ymax || lat <= (rast_ymax - rast_nrows * rast_yres)) {
      return; // outside raster extent — ignore
    }
    const col  = Math.floor((lng - rast_xmin) / rast_xres) + 1;
    const row  = Math.floor((rast_ymax - lat)  / rast_yres) + 1;
    const cell = (row - 1) * rast_ncols + col;
    notify(String(cell));
  }
  window.addEventListener('leaflet_mapclick', handler);
  notify(""); // initial: nothing selected
  return () => window.removeEventListener('leaflet_mapclick', handler);
})
```

```{ojs}
//| output: false
// Dispatch year change events to the map when hotspot_year changes
{
  window.dispatchEvent(new CustomEvent('hotspot_year_change', { detail: { year: hotspot_year } }));
}
```

### Cell trend over time {height="240px"}

```{r cell-trend-data, include=FALSE}
# Build per-variable cell trend (species count per cell × year × variable).
# Computed directly from exposure_df so we don't need to re-run prepare-data.
cell_trend_by_var <- exposure_df |>
  dplyr::filter(!is.na(cell), cell >= 1, cell <= terra::ncell(tpl)) |>
  dplyr::mutate(var = vapply(as.character(var), var_label, character(1))) |>
  dplyr::distinct(spName, cell, year, var) |>
  dplyr::count(cell, year, var, name = "n_species_exposed")
ojs_define(cell_trend_data = cell_trend_by_var)
```

```{ojs}
//| output: false
cell_trend_filtered = {
  if (clicked_cell === "") return [];
  const raw = transpose(cell_trend_data)
    .filter(d => d.cell === Number(clicked_cell))
    .filter(d => hotspot_vars.includes(d.var));
  // Build lookup: "year|var" -> n_species_exposed
  const lookup = new Map(raw.map(d => [`${d.year}|${d.var}`, d.n_species_exposed]));
  // Cross-join recent years (>2010) × selected vars, filling missing with 0
  const allYears = years_list.map(Number).filter(y => y > 2010);
  const rows = [];
  for (const year of allYears) {
    for (const v of hotspot_vars) {
      rows.push({ year, var: v, n_species_exposed: lookup.get(`${year}|${v}`) ?? 0 });
    }
  }
  return rows;
}
```

```{ojs}
//| echo: false
{
  if (clicked_cell === "" || cell_trend_filtered.every(d => d.n_species_exposed === 0))
    return html`<p style="color:#888;font-style:italic">Click any location on the map to see which species are exposed there.</p>`;
  const c = Number(clicked_cell);
  const col = (c - 1) % rast_ncols + 1;
  const row = Math.floor((c - 1) / rast_ncols) + 1;
  const lonRaw = rast_xmin + (col - 0.5) * rast_xres;
  const latRaw = rast_ymax - (row - 0.5) * rast_yres;
  const lon = Math.abs(lonRaw).toFixed(2) + "\u00b0" + (lonRaw >= 0 ? "E" : "W");
  const lat = Math.abs(latRaw).toFixed(2) + "\u00b0" + (latRaw >= 0 ? "N" : "S");
  const colorDomain = hotspot_vars;
  const colorRange  = hotspot_vars.map(v => var_label_colors[v] ?? "#cccccc");
  const legendWidth = 210;
  const containerW  = document.querySelectorAll('#hotspot-explorer .bslib-card')[1]
                        ?.querySelector('.card-body')?.clientWidth ?? 800;
  const chartWidth  = Math.max(150, containerW - legendWidth - 26);
  const chart = Plot.plot({
        width: chartWidth,
        height: 175,
        marginTop: 8,
        marginBottom: 40,
        style: { fontSize: "12px" },
        fx: { label: "Year", tickFormat: d => d },
        x:  { axis: null },
        y:  { label: "↑ Species", grid: true },
        color: { domain: colorDomain, range: colorRange, legend: false },
        marks: [
          Plot.barY(cell_trend_filtered, {
            fx: d => String(d.year),
            x: "var",
            y: "n_species_exposed",
            fill: "var",
            tip: true
          }),
          Plot.ruleY([0])
        ]
      });
  const legendEl = html`<div style="font-size:11px;line-height:1.3;padding-top:8px">${
    colorDomain.map((v, i) => html`<div style="display:flex;align-items:center;gap:5px;margin-bottom:5px">
      <span style="display:inline-block;width:11px;height:11px;flex-shrink:0;background:${colorRange[i]}"></span>
      <span>${v}</span>
    </div>`)
  }</div>`;
  return html`<div style="display:flex;gap:8px;align-items:flex-start;overflow:hidden">
    <div style="flex-shrink:0">${chart}</div>
    <div style="flex-shrink:0;width:${legendWidth}px">${legendEl}</div>
  </div>`;
}
```

### Species in selected cell

```{ojs}
//| output: false
// Compute formatted lat/lon from clicked cell index
click_latlon = {
  if (clicked_cell === "") return "";
  const c = Number(clicked_cell);
  const col = (c - 1) % rast_ncols + 1;
  const row = Math.floor((c - 1) / rast_ncols) + 1;
  const lonRaw = rast_xmin + (col - 0.5) * rast_xres;
  const latRaw = rast_ymax - (row - 0.5) * rast_yres;
  const lon = Math.abs(lonRaw).toFixed(2) + "\u00b0" + (lonRaw >= 0 ? "E" : "W");
  const lat = Math.abs(latRaw).toFixed(2) + "\u00b0" + (latRaw >= 0 ? "N" : "S");
  return `${lat}, ${lon}`;
}
```

```{ojs}
//| output: false
// Load the three small lookup files once (not re-fetched when year changes).
cell_species_lookups = Promise.all([
  fetch("data/cell_species/var_codes.json").then(r => r.json()),
  fetch("data/cell_species/redlist_codes.json").then(r => r.json()),
  fetch("data/cell_species/species_attrs.json").then(r => r.json()),
])
```

```{ojs}
//| output: false
// Fetch the species exposure JSON for the selected year lazily.
// Only the current year's compact file is loaded; others are fetched on demand.
cell_species = {
  const raw = await fetch(`data/cell_species/species_${hotspot_year}.json`).then(r => r.json());
  const [varCodes, redlistCodes, spAttrs] = await cell_species_lookups;
  return raw.map(d => ({
    cell:            d.c,
    spName:          d.s,
    var:             varCodes[d.v],
    propExposed:     d.p,
    redlistCategory: redlistCodes[d.r] ?? "—",
    orderName:       spAttrs[d.s]?.o   ?? "—",
    familyName:      spAttrs[d.s]?.f   ?? "—"
  }));
}
```

```{ojs}
//| output: false
cell_filtered = clicked_cell === ""
  ? []
  : Array.from(cell_species
      .filter(d => d.cell === Number(clicked_cell))
      .filter(d => hotspot_vars.includes(d.var))
      .filter(d => !hotspot_threatened_only || ["CR", "EN", "VU"].includes(d.redlistCategory))
      .reduce((bySpecies, row) => {
        const existing = bySpecies.get(row.spName);
        if (existing) {
          existing.variables.push(row.var);
          existing.maxPropExposed = Math.max(existing.maxPropExposed, row.propExposed ?? 0);
        } else {
          bySpecies.set(row.spName, {
            ...row,
            variables: [row.var],
            maxPropExposed: row.propExposed ?? 0
          });
        }
        return bySpecies;
      }, new Map())
      .values())
      .map(row => ({
        ...row,
        variables: hotspot_vars.filter(variable => row.variables.includes(variable)),
        propExposed: row.maxPropExposed
      }))
```

```{ojs}
//| echo: false
clicked_cell === ""
  ? html`<p style="color:#888;font-style:italic">Click any location on the map to see which species are exposed there.</p>`
  : html`<div>
    <h6 style="margin:0 0 6px">Species exposed in the selected grid cell in ${hotspot_year} (centre: ${click_latlon})</h6>
    ${cell_filtered.length === 0
      ? html`<p style="color:#888;font-style:italic">No species match the selected filters.</p>`
      : Inputs.table(
          cell_filtered.map(({ spName, variables, propExposed, orderName, familyName, redlistCategory }) => ({
            Species:           spName,
            "Exposed variables": variables.join("; "),
            "Max. range exposed": propExposed ? (propExposed * 100).toFixed(2) + "%" : "—",
            Order:             orderName   ?? "—",
            Family:            familyName  ?? "—",
            "IUCN status":     redlistCategory ?? "—"
          })),
          { width: "100%", sort: "Max. range exposed", reverse: true }
        )}
    </div>`
```
